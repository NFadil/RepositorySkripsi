...
class Informatika extends RestController {
    ...
    public function index_get($id = 0){
        $chek_data = $this->db->get_where('informatika',['id_jurnal'=> $id])->row_array();
        if($id){
            if($chek_data){
                $data = $this->db->get_where('informatika',['id_jurnal'=> $id])->row_array();
                $this->response($data,RestController::HTTP_OK);
            }else{
            $this->respone(['status' => false,'massage'=>'Data Tidak temukan'],404);
            }
        }else{
            $data = $this->db->get('informatika')->result();
            $this->response($data,RestController::HTTP_OK);
        }
    ...
    public function index_post()
    {
        ...
        $insert = $this->db->insert('informatika', $data);
        if ($insert) {
            $this->response($data, RestController::HTTP_OK);
        } else {
    ...
    public function index_put(){
        ...
        $update = $this->db->update('informatika', $data);
        if ($update) {
            $this->response($data, RestController::HTTP_OK);
        } else {
            $this->response(array('status' => 'failed', RestController::HTTP_INTERNAL_SERVER_ERROR));
